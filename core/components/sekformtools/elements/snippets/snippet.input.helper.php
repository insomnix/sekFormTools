 ...
 */
$helper_snippet = $_REQUEST['hs'];

if($helper_snippet > ''){
    if ($modx->getObject('modSnippet',array('name' => $helper_snippet))) {
        $return = $modx->runSnippet($helper_snippet);
    } else {
        $modx->log(modX::LOG_LEVEL_ERROR,'[sekFormTools] Could not load snippet: '.$helper_snippet);
    }
}else{
    $return = $modx->runSnippet('helper.filter');
}
